...
- Human → Earth uses two bridge screens plus an in-scene Earth comparison, using fixed 340 m and 70 km milestones, with ratios 1:200, ~1:206, and ~1:182.
...
- Solar neighborhood → Milky Way uses the fixed 0.5 kpc intermediate comparison.
...
Milky Way now adopts a representative 30 kpc stellar disk (ESA; no unique edge). Neighborhood ↔ Milky Way uses one standalone comparison, 8 pc → 0.5 kpc (user-adopted intermediate length). The final 0.5 kpc → 30 kpc comparison is inside the galaxy; the ratios are 62.5 and 60. See `scenes.md` §8 for physical plane and camera conventions.
