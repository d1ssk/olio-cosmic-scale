...
Use a representative 1.7 m reference bar in a local Y-up coordinate frame, with 0.17 m per render unit. The requested Hachikō scan by Maurice Svay (CC BY 4.0) is the human-scale object. Its metric calibration is not supplied by the author: normalize the entire imported model, including its base, to an explicitly labeled adopted height of 2.17 m, supplied by the user on 2026-09-11 (not independently verified). Keep the representative human reference bar at 1.7 m. Preserve proportions, center X/Z, and place the model base at Y=0. Place the reference bar outside the model’s positive-Z bounding edge (the scan’s lateral direction; its head faces approximately −X), with 0.25 m of physical clearance; its base stays at Y=0. Source and license links remain visible in both languages and on mobile.
...
The default camera is (0,32,40) looking at the origin, about 39° above the plane, with a 40 kpc short-side viewport extent. X-parallel rulers sit in the Galactic plane: 30 kpc at Z=17 kpc and the 0.5 kpc comparison at Z=19 kpc, both centered on X=0. Thus both are horizontal and below the disk at reset, fixed in world space through orbiting. Physical ruler lengths and disk thickness are not exaggerated. The prior 8 pc → 30 kpc interval has one standalone bridge, 8 pc → 0.5 kpc; the second comparison happens within the galaxy. Both directions use the existing sequential transfer, hidden-bar restoration and reduced-motion behavior, including entry from the bulge sibling. Local Group remains a placeholder.
...
The supplied precomputed past-light-cone wedge now contains two GPU point layers: a faint matter-density proxy and compact biased luminous tracers. Its metadata owns the 26° opening, 0.196 Gpc thickness, 14 Gpc extent, formats and counts. One right-handed parent transform aligns its X/Y/Z with the CMB SIDE/UP/AXIS basis at the existing SI normalization. A double-sided dark midplane aids visibility from above and below. The restored translucent sector and both point layers share a visualization-only Y-thickness multiplier of 0.25 (0.049 Gpc displayed versus 0.196 Gpc in the source), disclosed in the UI. Both layers have independent bilingual checkboxes, initially on. This is the supplied synthetic model from `tmp/observable_universe_wedge/`, replacing the nearby-boost variant, not an observed catalog; the user-requested tracer-only display gradient smoothly increases size and brightness toward the observer, with no changes to source sampling or matter. See the scene README.
