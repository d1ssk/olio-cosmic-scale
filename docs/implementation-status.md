...
| `earth` — 地球 / Earth                   | Spherical Earth, mean diameter 12,742 km; local NASA monthly texture                                                                             | Diameter bar beside Earth over the Pacific; 70 km comparison bar alongside it                                 |
...
| `milky-way`                              | Sourced representative disk and Sun distance; deterministic schematic arms and bulge/bar; oblique camera                                         | 30 kpc and 0.5 kpc horizontal bars in the foreground disk plane; one standalone bridge from the neighborhoods |
...
- Ratios up to 200 bypass standalone bridges. Larger intervals use geometrically spaced comparisons unless a fixed user-adopted milestone is specified. Human–Earth has two standalone comparisons (1.7 m → 340 m → 70 km); the final comparison occurs inside Earth.
...
Fixed the missing bridge animation target on Milky Way Previous: the bridge contains 0.5 kpc, not the scene's 30 kpc reference. Entry now selects the first/last planned physical length by direction. New component tests verify actual Web Animation keyframes, hidden target, post-expansion comparison reveal and lock release across three reverse edges. Total: 98 app tests. Browser measurements confirm ~9.5 px → ~1,233 px continuous expansion at 1440×1000 and onward navigation to the Solar neighborhood.
